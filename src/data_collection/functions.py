...
def _get_secret(project_id: str, sa_key_name='service-account-key'):
...
def create_client(project_id: str, sa_key_name: str):
    key = _get_secret(project_id, sa_key_name)
    creds = service_account.Credentials.from_service_account_info(key)
    client = bigquery.Client(credentials=creds, project=creds.project_id)
    return client

def write_to_BQ(client: bigquery.Client, table_name: str, dframe: pd.DataFrame, dataset='Kaggle_test_data', disp = "WRITE_APPEND", schema_name='schema.json'):
...
def get_data_from_BQ(bq_client: bigquery.Client, TARGET_PROJECT_ID: str, DATASET: str, source_table: str):
...
def update_raw_data(client: bigquery.Client, query: str):
